...
variable and breaks `simp`'s congruence/motive machinery. In the backward-map
membership obligation `Finset.card_bij'` presents the constructed pair as the
un-beta-reduced application `((fun y _ => (y, (x * y)⁻¹)) y hy)`, so its `.2`
projection still hides the `(x * y)⁻¹` subterm and a literal `rw [mul_inv_rev]`
fails to match. We beta-reduce the goal with `show y * (x * y)⁻¹ = x⁻¹` first and
discharge it with `group`, which normalizes the identity directly. The left-inverse
goal is likewise beta-reduced with `show` before `rw [hp2]`.
-/
