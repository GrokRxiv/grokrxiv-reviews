...
\fancyhf{}
\fancyfoot[C]{\thepage}
\title{A Deterministic Agentic Workflow for HS Tariff Classification: Multi-Dimensional Rule Reasoning with Interpretable Decisions}
\date{}
\begin{document}
\maketitle

\noindent\textbf{arXiv:} \href{https://arxiv.org/abs/2605.14857}{2605.14857} \quad \textbf{Field:} cs.AI\\
...
\begin{itemize}[leftmargin=*]
  \item Addresses a practically significant and underexplored problem—multi-dimensional rule reasoning for HS tariff classification—with a well-motivated architectural design rooted in the General Interpretive Rules of the Harmonized System.
  \item The deterministic workflow concept (fixed control flow, narrow LLM calls, local reflection/verification) is a principled and differentiating design choice relative to autonomous self-planning agents, offering interpretability by construction through stage-wise structured outputs with verbatim rule citations.
  \item Empirical results appear competitive, and the finding that a smaller open-weight model achieves performance close to a frontier model suggests the approach is model-agnostic and practically deployable.
  \item The benchmark audit raising questions about HSCodeComp ground-truth label quality is a valuable contribution to the community, potentially prompting dataset curation efforts.
  \item Novelty is judged significant: the paper introduces a domain-specific deterministic agentic architecture for a high-stakes regulatory classification task, with clear differentiation from prior end-to-end and self-planning approaches.
\end{itemize}

\section*{Weaknesses}
\begin{itemize}[leftmargin=*]
  \item Model identifiers appear incorrect or at least undocumented: 'Qwen3.6-plus' and 'Qwen3.6-27B-FP8' do not correspond to known public model releases, undermining reproducibility and the credibility of reported numbers.
  \item Reproducibility is severely limited: no code repository, no dataset or knowledge-base URL, no prompt templates, no decoding hyperparameters, no retrieval/indexing parameters, and no compute environment details are provided.
  \item Key empirical claims are insufficiently substantiated: the performance numbers cannot be independently verified, the distinction between inter-model agreement and ground-truth accuracy is blurred for the open-weight model results, and the benchmark audit lacks inter-annotator reliability statistics and auditor credential disclosure.
  \item The claim that end-to-end prompting 'characteristically fails' at multi-dimensional rule reasoning is presented without a systematic error taxonomy or ablation study comparing the proposed workflow directly against the baseline.
  \item The interpretability-by-construction claim is overstated without faithfulness evaluations (e.g., citation-grounding checks or counterfactual analysis) demonstrating that structured outputs faithfully reflect actual model decisions.
  \item No bibliography was provided for audit, and the related work section appears to lack discussion of relevant prior art including PAL-style structured reasoning frameworks, classical expert systems for regulatory classification, and non-LLM ML approaches to HS coding.
  \item Evaluation protocol details—HSCodeComp split, preprocessing, label normalization, top-k computation, and handling of audited disagreements—are not specified.
\end{itemize}

\section*{Open Questions}
\begin{itemize}[leftmargin=*]
  \item Can the authors confirm and correct the model names used (Qwen3.6-plus and Qwen3.6-27B-FP8)? Please provide exact HuggingFace identifiers or API version strings to enable reproducibility.
  \item Will code, prompts, decoding configurations, retrieval/indexing parameters, and the offline Chinese HS tariff knowledge base be released? If so, on what timeline and under what license?
  \item For the open-weight model results, does the reported 77.4\% figure represent top-1 accuracy against HSCodeComp ground truth or inter-model agreement with the frontier model? Please clarify and report both metrics.
  \item Can the authors provide an ablation study or error taxonomy comparing end-to-end prompting directly against the proposed deterministic workflow to substantiate the characterization of LLM failure modes?
  \item For the benchmark audit, what are the inter-annotator agreement statistics (e.g., Cohen's kappa), auditor qualifications, and adjudication methodology? What fraction of the 226 audited labels were judged incorrect, and with what confidence?
  \item How does the proposed approach relate to structured reasoning frameworks such as PAL, and to classical expert systems or rule-based systems for legal/regulatory classification? A related work section addressing these connections is expected.
  \item What are the exact HSCodeComp dataset split details, label normalization procedures, and top-k evaluation criteria used?
\end{itemize}
