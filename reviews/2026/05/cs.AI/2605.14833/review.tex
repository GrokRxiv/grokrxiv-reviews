...
\fancyhf{}
\fancyfoot[C]{\thepage}
\title{Emotion-Attended Stateful Memory (EASM):The Architecture for Hyper-Personalization at Scale}
\date{}
\begin{document}
\maketitle

\noindent\textbf{arXiv:} \href{https://arxiv.org/abs/2605.14833}{2605.14833} \quad \textbf{Field:} cs.AI\\
...
\begin{itemize}[leftmargin=*]
  \item Addresses a genuine and important problem: the fundamental statelessness of production LLMs and the consequent inability to build emotionally-aware, persistent personalization across sessions.
  \item The EASM conceptual architecture—combining long-term history, emotional signals, and inferred intent for dynamic context construction at inference time—is a coherent and practically motivated design.
  \item Experimental design uses non-scripted, emotionally diverse conversations (six categories including adversarial scenarios such as grief and distress), which is more ecologically valid than scripted benchmarks.
  \item Authors explicitly acknowledge limitations and call for broader validation, demonstrating appropriate scientific humility.
  \item The framing of stateful emotional memory as infrastructure rather than a feature is a useful conceptual contribution to the discourse on AI system design.
  \item Novelty assessment by specialists rates the contribution as 'significant' relative to RAG, fine-tuning, and generative agent baselines, particularly in the combination of emotion-attended and stateful personalization at inference time.
\end{itemize}

\section*{Weaknesses}
\begin{itemize}[leftmargin=*]
  \item Evaluation is critically underpowered: N=30 conversations (\~{}5 per emotional category) is insufficient for robust statistical inference, and no significance tests, confidence intervals, or effect-size estimates are reported.
  \item The 'memory grounding' metric is potentially tautological—any memory-enabled system will trivially score higher than a stateless baseline on a metric measuring whether responses reference past history—raising serious concerns about construct validity.
  \item No bibliography or citation list is included in the submitted material; claims drawing on psychological foundations, prior architectural work, and benchmark literature are entirely uncited.
  \item Core architectural details are absent: the underlying language model, emotion classifier, memory schema, retrieval policy, embedding approach, intent inference method, and prompting strategy are not specified, making independent replication impossible.
  \item No code, dataset, annotation rubrics, or evaluation transcripts are shared or indicated as available; reproducibility score is rated 1.5/10 by specialists.
  \item Rater protocol is underspecified: blinding, inter-rater reliability (e.g., Cohen's kappa), rubric definitions for each metric, and randomization procedures are not described.
  \item Key quantitative claims (95\%, 57\%, 34\% improvements) are reported without absolute baseline scores, per-category breakdowns, or variance estimates, making them difficult to contextualize.
  \item The strong infrastructural claim—that EASM is 'foundational infrastructure for hyper-personalized AI'—is not empirically established by an N=30 A/B study and should be framed as a position or hypothesis.
  \item Insufficient engagement with related work: existing longitudinal memory benchmarks (LongMemEval, LoCoMo, MemoryBank), recurrent memory architectures, and personalized dialogue systems are not cited or contrasted.
  \item The claim that RAG and fine-tuning cannot enable persistent personalization is overstated; RAG over user-specific histories is a common personalization strategy that deserves more nuanced treatment.
\end{itemize}

\section*{Open Questions}
\begin{itemize}[leftmargin=*]
  \item Can you provide full algorithmic and implementation details for EASM, including the underlying LLM, emotion classification method, memory schema, retrieval policy, context construction rules, and prompting strategy?
  \item Will code, anonymized conversation transcripts, and annotation rubrics be made publicly available? If not, can synthetic equivalents be provided to support reproducibility?
  \item How is 'memory grounding' defined operationally, and how do you address the concern that this metric tautologically favors memory-enabled systems? What steps were taken to ensure construct validity?
  \item What statistical tests were performed, and can you report absolute scores, per-category breakdowns, confidence intervals, and effect sizes for all three metrics?
  \item What was the inter-rater reliability (e.g., Cohen's kappa or Krippendorff's alpha) for the human evaluators, and were evaluators blind to condition assignment?
  \item How does EASM compare to or differentiate from existing longitudinal memory benchmarks and architectures (e.g., LongMemEval, LoCoMo, MemoryBank, Recurrent Memory Transformers)?
  \item Can you provide citations for the psychological claims (e.g., the claim that emotional state and memory are 'inseparable'), and engage with the affective computing and empathetic dialogue literature?
  \item With only \~{}5 conversations per emotional category, how do you support robustness claims in adversarial emotional scenarios? Is expansion of the adversarial subset planned?
  \item How does EASM handle privacy, consent, and data security concerns given that it stores persistent emotional and behavioral user histories across sessions?
  \item Can you provide ablation experiments isolating the contribution of the emotional signal component from the long-term memory component alone?
\end{itemize}
